...
import ru.geekbrains.lesson4.task3.exceptions.BuyingException;
import ru.geekbrains.lesson4.task3.exceptions.TicketNotFoundException;
import ru.geekbrains.lesson4.task3.exceptions.WrongCardFormatException;
...
class TicketProvider{
    ...
    /**
     * по переданному clientId находит билеты клиента, и, если так же дата билета соответствует искомой, то возвращает
     * в коллекции tickets. В случае, если билеты не были найдены, получаем исключение TicketNotFoundException
     * @param clientId id клиента, чьи билеты ищем
     * @param date дата искомых билетов
     * @return коллекцию тикетов
     */
    ...
    /**
     * Проверяет формат номера карты (номер состоит из 16 цифр, в ином случае нас ждёт WrongCardFormatException),
     * формирует заказ билета в database и обращается к paymentProvider для осуществления транзакции.
     * @param clientId
     * @param cardNo
     * @return true, если операция прошла, или BuyingException - если нет.
     */
    public boolean buyTicket(int clientId, String cardNo){

        if (cardNo.length() != 16) {
            throw new WrongCardFormatException("Wrong card number!");
        }

        int orderId = database.createTicketOrder(clientId);
        double amount = database.getTicketAmount();
        boolean isBuying = paymentProvider.buyTicket(orderId,  cardNo, amount);
        if (!isBuying) {
            throw new BuyingException("Operation is not complete!");
            } else return isBuying;

    }

    /**
     * по заданному qr-коду ищет соответствие в коллекции tickets database. Если нету соответствия, возвращает TicketNotFoundException
     * @param qrcode
     * @return
     */
    public boolean checkTicket(String qrcode){
        ...
        throw new TicketNotFoundException("There's no ticket with this QR-code.");
    }
